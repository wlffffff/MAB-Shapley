...
from utils.shapley import all_subsets, list2str, cal_shapley, cal_mab_stratified_shapley_0, cal_mab_stratified_shapley_t, shapley_rank
...
if __name__ == '__main__':
    args = args_parser()
    args.method = "mab_stratified_mc_shapley"
    ...
    for iter in range(args.epochs):
        ...
        if iter == 0:             # 第一轮需要采样所有排列，因此需要聚合所有可能的子模型，并获得其准确率
            ...
            # 更新SVR, T, B
            SVR += sum_ratio
            sample_size = normal_distribution(SVR)
            for j in range(args.num_users):
                T[j] += 1
            ...
            for i in index:
                iteration_list.append(sample_size_new[i])
            ...
            for client in range(args.num_users):
                phi_client = []
                for j in index:
                    shapley_dict[int(client)] += 1/len(index) * (mean_func(Contribution_records[j][client]))
                    phi_client.append(1/len(index) * mean_func(Contribution_records[j][client]))
    ...
    with open('./shapley_result/running_time.txt', 'a') as f:
        f.write('Mab_stratified_mc_shapley: client number_' + str(args.num_users) + ' selected strat_' + str(int(args.num_users* args.para_K)) + ' para_H_' + str(args.para_H) + " Para_iteration_" + str(args.iteration) +
