...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):
        ...
        if iter == 0:             # 第一轮需要采样所有排列，因此需要聚合所有可能的子模型，并获得其准确率
            ...
            print(f"轮次{iter}中每个分层的重要性sum_ratio: {sum_ratio}")
            ...
            print(f"轮次{iter}中每个分层的重要性sum_ratio: {new_sum_ratio}")
    ...
    #     f.write('Mab_stratified_shapley: client number_' + str(args.num_users) + ' selected strat_' + str(int(args.num_users* args.para_K)) + ' running_time: ' + str(execution_time) + 's'+'\n')
